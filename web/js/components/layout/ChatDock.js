...
const { MoreHorizontal } = window;
const { MOCK_CHATS, MOCK_COMMUNITIES } = window;
...
const renderChatTabs = (activeTab, setActiveTab) => (
...
const renderChatListItems = (chats, activeTab, selectedChat, setSelectedChat) => (
...
const renderChatWindowContent = (selectedChat, activeTab, MoreHorizontalIcon) => (
    <>
        {/* Chat Header */}
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {/* --- MAXIMIZED: Side-by-side (list + chat) --- */}
                {isMaximized ? (
                    <>
                        {/* Chat List (always visible when maximized) */}
                        <div className="w-72 border-r border-stone-100 flex flex-col bg-white flex-shrink-0">
                            {renderChatTabs(activeTab, setActiveTab)}
                            {renderChatListItems(chats, activeTab, selectedChat, setSelectedChat)}
                        </div>

                        {/* Chat Window or Empty State */}
                        {selectedChat ? (
                            <div className="flex-1 flex flex-col bg-stone-50 animate-enter min-w-0">
                                {renderChatWindowContent(selectedChat, activeTab, MoreHorizontal)}
                            </div>
                        ) : (
                            <div className="flex-1 flex flex-col items-center justify-center text-tab-muted gap-4 bg-stone-50">
                    ...
                    /* --- NORMAL (40%): List OR Chat, never both --- */
                    selectedChat ? (
                        <div className="flex-1 flex flex-col bg-stone-50 animate-enter min-w-0">
                            {/* Back button to return to list */}
                            <div className="h-12 border-b border-stone-200 bg-white flex items-center px-3 flex-shrink-0">
