...
// Bridge PHP → JS: prepare posts data for React
...
$jsExternal = json_encode($external ?? []);
?>
...
<script>
    ...
    window.__INITIAL_DATA__ = {
        posts: <?= $jsPosts ?>,
        external: <?= $jsExternal ?>,
        authHash: null, // will be read from localStorage in React
        urls: {
            createPost: '<?= Yii::$app->urlManager->createUrl(["post/create"]) ?>',
            home: '<?= Yii::$app->homeUrl ?>',
        },
        isGuest: <?= Yii::$app->user->isGuest ? 'true' : 'false' ?>,
        currentUser: <?= Yii::$app->user->isGuest ? 'null' : json_encode([
...
<script type="text/babel">
    ...
    const MOCK_USER = {
        ...
        bio: "Criador do TabNews. \n\nO valor concreto é o que importa. Programação, Newsletter e uma comunidade que constrói junto.\n\nFocando em tecnologia, não em hype.",
        location: "Brasil",
    ...
    // Merge PHP data + mock data for rich demo
    const phpPosts = (window.__INITIAL_DATA__?.posts || []).map((p, i) => ({
        id: 'db-' + p.id,
        type: 'tabet',
        user: p.username,
        handle: p.username,
        time: new Date(p.created_at * 1000).toLocaleString('pt-BR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
        content: p.content,
        stats: { comments: 0, reposts: 0, score: p.points },
        verified: false,
        commentsData: []
    }));
    ...
    const DEMO_POSTS = [
        {
            ...
            handle: "akitaonrails",
            time: "2h",
            content: "Não existe bala de prata. O problema é que a maioria dos devs quer uma solução mágica para complexidade acidental que eles mesmos criaram. #arquitetura #realidade #backend",
            stats: { comments: 42, reposts: 12, score: 156 },
            verified: true,
            commentsData: [
                { user: "dev_junior", handle: "jr_dev", content: "Mas e o microservices?", time: "1h" },
                { user: "akitaonrails", handle: "akitaonrails", content: "Você tem escala do Netflix? Não? Então faça um monólito bem feito.", time: "55m" }
            ]
        },
        {
            id: 2,
            type: 'article',
            title: "Por que reescrevemos o Kernel em Rust",
            user: "torvalds",
            handle: "linux_god",
            time: "4h",
            content: "A decisão não foi fácil. Durante anos, C foi a linguagem soberana do desenvolvimento de sistemas. No entanto, a segurança de memória e a ergonomia moderna do #Rust provaram ser irresistíveis.\n\nNeste artigo, detalho como a abstração de custo zero permitiu que mantivéssemos a performance enquanto eliminávamos classes inteiras de bugs. Começamos com drivers de dispositivos, onde a complexidade é isolada, e gradualmente movemos subsistemas críticos.\n\nO resultado? Menos 'kernel panics', maior manutenibilidade e uma barreira de entrada menor para novos contribuidores que não querem lidar com ponteiros selvagens. #kernel #linux",
            stats: { comments: 89, reposts: 340, score: 5012 },
            verified: true,
            commentsData: []
        },
        {
            ...
            handle: "ada_lovelace",
            time: "5h",
            content: "Acabei de publicar um novo algoritmo para calcular números de Bernoulli. O link está na minha bio. #math #algoritmos #historia",
            stats: { comments: 5, reposts: 2, score: 30 },
            verified: false,
            commentsData: []
        }
    ];

    const INITIAL_POSTS = [...phpPosts, ...DEMO_POSTS];
    ...
    // --- Tab Config Modal ---
    const TabConfigModal = ({ onClose, onSave }) => {
        ...
        return (
            <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-enter p-4">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 border border-stone-200">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold text-stone-900">Nova Aba</h3>
                        <button onClick={onClose} className="p-1 hover:bg-stone-100 rounded-full">{X && <X size={18} />}</button>
                    </div>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-stone-700 mb-1">Nome da Aba</label>
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-tab-accent" placeholder="Ex: DevOps" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-stone-700 mb-1">Filtrar por tipo</label>
                            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-tab-accent bg-white">
                                <option value="all">Todos</option>
                                <option value="tabet">Posts</option>
                                <option value="article">Artigos</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-stone-700 mb-1">Tag (opcional)</label>
                            <input type="text" value={tag} onChange={(e) => setTag(e.target.value)} className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-tab-accent" placeholder="Ex: rust" />
                        </div>
                    </div>
                    <div className="flex justify-end gap-3 mt-6">
                        <Button onClick={onClose}>Cancelar</Button>
                        <Button primary onClick={() => onSave({ name, filterType, tag })} disabled={!name.trim()}>Criar Aba</Button>
                    </div>
                </div>
            </div>
        );
    ...
    const VoteWidget = ({ score, isArticle, onVoteAttempt }) => {
        const [localScore, setLocalScore] = useState(score);
        const [voteState, setVoteState] = useState(0);

        const handleVote = (direction, e) => {
            e.stopPropagation();
            if (isArticle) { onVoteAttempt(); return; }
            if (voteState === direction) { setVoteState(0); setLocalScore(score); }
            else { setVoteState(direction); setLocalScore(score + direction); }
        };
        ...
        return (
            <div className="flex flex-col items-center gap-1 bg-stone-50 rounded-lg p-1 mr-4 min-w-[36px] border border-stone-100 h-fit">
                ...
                <button onClick={(e) => handleVote(-1, e)} className={`p-1 rounded hover:bg-stone-200 transition-colors ${voteState === -1 ? 'text-tab-danger bg-red-50' : 'text-stone-400'}`}>
                    <IconChevronDown />
                </button>
            </div>
        );
        ...
        return (
            <div className="animate-enter">
                <div className="mb-4 sticky top-[60px] bg-tab-bg z-10 py-2">
                    ...
                    <h1 className="text-2xl font-bold text-tab-text flex items-center gap-2">{tag}</h1>
    ...
    const MainApp = () => {
        ...
        const handleNewPost = () => {
            ...
            const urls = window.__INITIAL_DATA__?.urls;

            if (authHash && urls?.createPost) {
                fetch(urls.createPost, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ auth_hash: authHash, content: newPostContent })
                }).then(r => r.json()).then(data => {
                    if (data.status === 'ok') {
                        const newPost = {
                            id: 'new-' + data.id,
                            type: 'tabet',
                            user: window.__INITIAL_DATA__?.currentUser?.username || 'me',
                            handle: window.__INITIAL_DATA__?.currentUser?.username || 'me',
                            time: 'agora',
                            content: newPostContent,
                            stats: { comments: 0, reposts: 0, score: 1 },
                            verified: false,
                            commentsData: []
                        };
        ...
        const handleCommentClick = (post) => {
            ...
            else { setSelectedPost(post); setView('post'); }
        };
        ...
        return (
            <div className="min-h-screen flex justify-center bg-tab-bg text-tab-text font-sans">
                ...
                {showTabModal && <TabConfigModal onClose={() => setShowTabModal(false)} onSave={handleCreateTab} />}
                {modalConfig && <Modal {...modalConfig} />}

                {/* Left Sidebar */}
                {showLeftSidebar && <div className="fixed inset-0 bg-black/20 z-40 sm:hidden" onClick={() => setShowLeftSidebar(false)}></div>}
                <aside className={`
                    fixed left-0 top-0 h-screen bg-white border-r border-tab-border z-50 transition-all duration-300
                    ${showLeftSidebar ? 'w-64 translate-x-0' : '-translate-x-full w-0 opacity-0'}
                    overflow-hidden
                `}>
                    <div className="p-6 h-full flex flex-col w-64">
                        ...
                        <nav className="flex flex-col gap-2">
                            ...
                            <div onClick={() => { setView('profile'); setShowLeftSidebar(false); }} className={`flex items-center gap-4 p-3 rounded-lg cursor-pointer ${view === 'profile' ? 'bg-stone-100 font-bold' : 'hover:bg-stone-50 text-tab-muted'}`}>
                        ...
                        <div className="p-4 sm:p-6">
                            {view === 'feed' && (
                                <>
                                    ...
                                    {/* Create Post */}
                                    <div className="bg-white border border-tab-border rounded-lg p-4 mb-6 shadow-sm">
                                        <div className="flex gap-4 items-start">
                                            ...
                                            <div className="flex-1">
                                                <textarea
                                                    ...
                                                    className="w-full text-sm resize-none border-0 focus:outline-none bg-transparent min-h-[60px]"
                                                    rows={2}
                                                />
                                                <div className="flex justify-end border-t border-stone-100 pt-3 mt-2">
                                                    <Button primary onClick={handleNewPost} disabled={!newPostContent.trim()}>Publicar</Button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                {/* Right Sidebar */}
                <aside className={`
                    fixed right-0 top-0 h-screen bg-white border-l border-tab-border z-40 transition-all duration-300
                    ${showRightSidebar ? 'w-80 translate-x-0' : 'translate-x-full w-0 opacity-0'}
                    overflow-hidden
                `}>
