...
$jsTrending = json_encode($trending ?? []);
...
<script type="text/babel">
    ...
    const TRENDING_TABS = window.__INITIAL_DATA__?.trending || [];
    ...
    const VoteWidget = ({ score, isArticle, onVoteAttempt, onVote, userVote }) => {
        // userVote: 0 (none), 1 (up), -1 (down - not implemented in backend yet)
        const [localScore, setLocalScore] = useState(score);
        const [voteState, setVoteState] = useState(userVote || 0);
        ...
        const handleVote = (direction, e) => {
            ...
            if (voteState === direction) {
                // Toggle off (not supported by backend strictly speaking for upvote toggle, but UI can show it)
                // Actually backend prevents double voting, so we might just block it.
                // For now let's assume simple upvote logic: Upvote or nothing.
                return;
            }
            ...
            setLocalScore(score + direction);
        };

        return (
            <div className="flex flex-col items-center gap-1 bg-stone-50 rounded-lg p-1 mr-4 min-w-[36px] border border-stone-100 h-fit">
                <button onClick={(e) => handleVote(1, e)} className={`p-1 rounded hover:bg-stone-200 transition-colors ${voteState === 1 ? 'text-tab-accent bg-green-50' : 'text-stone-400'}`} disabled={voteState === 1}>
                    <IconChevronUp />
                </button>
                <span className={`text-xs font-bold font-mono ${voteState === 1 ? 'text-tab-accent' : voteState === -1 ? 'text-tab-danger' : 'text-stone-600'}`}>
                    {localScore}
                </span>
                <button className="p-1 rounded text-stone-300 cursor-not-allowed">
                    <IconChevronDown />
                </button>
            </div>
        );
    ...
    const PostItem = ({ post, onCommentClick, onArticleClick, onArticleVoteAttempt, onPostClick, onHashClick, onVote }) => {
        return (
            <article
    ...
    const ProfileView = ({ user, onBack, onHashClick, onPostClick, onVote }) => {
    ...
    const HashtagView = ({ tag, onBack, onHashClick, onPostClick, onVote }) => {
    ...
    const MainApp = () => {
        ...
        const handleVote = (post, direction) => {
            if (direction !== 1) return; // Only upvotes for now

            const authHash = localStorage.getItem('auth_hash'); // or read from bridge if persisted
            // Note: backend checks auth via session or hash. 
            // We can use a simple fetch call.

            fetch(`<?= Yii::$app->homeUrl ?>post/upvote?id=${post.id.replace('db-', '')}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                ...
                if (data.status === 'ok') {
                    ...
                    setPosts(currentPosts => currentPosts.map(p => {
                        if (p.id === post.id) {
                            return {
                                ...p,
                                stats: { ...p.stats, score: data.points },
                                userVote: 1
                            };
                        }
                        return p;
